...
const FRESH_SESSION = { sessionId: null, questions: [], current: 0, attempts: {}, draft: "", step: "input", jd: "" };
...
export default function App() {
    ...
    try {
      const res = await fetch(`${API}/evaluate-answer`, {
        ...
        body: JSON.stringify({
          ...
          question_index: qIndex, question, answer: retryDraft,
        }),
      });
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const data = await res.json();
      setSess({ ...FRESH_SESSION, jd: sess.jd, sessionId: data.session_id, questions: data.questions, step: "questions" });
    } catch (err) {
      alert(err.message);
    } finally {
    ...
    try {
      const res = await fetch(`${API}/evaluate-answer`, {
        ...
        body: JSON.stringify({
          ...
          question_index: sess.current,
          question: sess.questions[sess.current],
          answer: sess.draft,
        }),
      });
      if (!res.ok) throw new Error("Evaluation failed");
      ...
      setSess(prev => ({
        ...
        attempts: {
          ...prev.attempts,
          [prev.current]: [...(prev.attempts[prev.current] || []), data],
        },
      }));
    } catch (err) {
      alert(err.message);
  ...
  const latestFeedback = (idx) => {
    const arr = sess.attempts[idx];
      ...
      <div style={s.authWrap}>
        <div style={s.authCard}>
          ...
          <form onSubmit={handleAuth} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {authMode === "signup" && (
              <input style={s.input} placeholder="Full name" value={authForm.name}
                onChange={e => setAuthForm(p => ({ ...p, name: e.target.value }))} />
            )}
            <input style={s.input} placeholder="Email" type="email" value={authForm.email}
              onChange={e => setAuthForm(p => ({ ...p, email: e.target.value }))} />
            <input style={s.input} placeholder="Password (min 6 chars)" type="password" value={authForm.password}
          ...
          <p style={{ color: "#555", fontSize: 13, marginTop: 16, textAlign: "center" }}>
            ...
            <span style={{ color: "#00e676", cursor: "pointer" }}
  ...
  if (showHistory) return (
    <div style={s.root}><div style={s.grid} />
      ...
      <main style={{ ...s.main, maxWidth: 900, display: "flex", gap: 28, alignItems: "flex-start" }}>
        {/* Sessions sidebar */}
        <div style={{ width: 220, flexShrink: 0 }}>
          <p style={s.sectionLabel}>Sessions</p>
          {sessions.length === 0 && <p style={{ color: "#444", fontSize: 13 }}>No sessions yet.</p>}
          {sessions.map(sess => (
            <div key={sess.id} onClick={() => loadSessionResults(sess.id)}
              style={{ ...s.histCard, borderColor: historyData?.session_id === sess.id ? "#00e676" : "#222" }}>
              <p style={{ color: "#ccc", fontSize: 13, marginBottom: 4, lineHeight: 1.4 }}>
                {sess.job_description.slice(0, 55)}…
              </p>
              <p style={{ color: "#444", fontSize: 11 }}>{new Date(sess.created_at).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {historyData && historyData.results.map((item, i) => {
            const lastAttempt = item.attempts[item.attempts.length - 1];
            const totalDelta = item.attempts.length > 1
              ? lastAttempt.score - item.attempts[0].score : null;
              ...
              <div key={i} style={{ ...s.resultCard, borderLeft: `3px solid ${vc?.color || "#333"}`, marginBottom: 20 }}>
                    ...
                    {item.attempts.map((a, ai) => {
                      ...
                      return (
                        <div key={ai} style={{ textAlign: "center" }}>
                          <div style={{ width: 36, height: 36, borderRadius: "50%", background: avc?.bg || "#222", border: `2px solid ${avc?.color || "#333"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: avc?.color }}>
                            {a.score}
                          </div>
                ...
                {retryingIndex === i ? (
                  <div style={{ marginTop: 16 }}>
                    <textarea style={{ ...s.answerBox, marginBottom: 10 }}
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <button style={{ ...s.primaryBtn, opacity: retryDraft.trim() ? 1 : 0.4 }}
                        disabled={!retryDraft.trim() || retryLoading}
                        onClick={() => handleRetry(i, item.question)}>
    ...
    <div style={s.root}><div style={s.grid} />
      <header style={s.header}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button style={s.ghostBtn} onClick={loadHistory} disabled={historyLoading}>
            {historyLoading ? "…" : "History"}
          </button>
          {sess.step !== "input" && (
            <button style={s.ghostBtn} onClick={() => setSess(FRESH_SESSION)}>+ New</button>
          )}
          <button style={s.ghostBtn} onClick={signOut}>Sign Out</button>
        </div>
      </header>
        ...
        <main style={s.main}>
            ...
            {sess.questions.map((_, i) => (
              <div key={i} onClick={() => setSess(p => ({ ...p, current: i, draft: "" }))}
                style={{ ...s.progressDot, cursor: "pointer",
                  background: latestFeedback(i)
                    ? VERDICT[latestFeedback(i).verdict]?.color || "#69f0ae"
          ...
          <div style={s.qCard}>
            ...
            <textarea style={s.answerBox}
              placeholder={(sess.attempts[sess.current] || []).length > 0
                ? "Try again with an improved answer…" : "Type your answer here…"}
              value={sess.draft}
              onChange={e => setSess(p => ({ ...p, draft: e.target.value }))}
              rows={5} />
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              {(sess.attempts[sess.current] || []).length > 0 && sess.current < sess.questions.length - 1 && (
                <button style={s.ghostBtn} onClick={() => setSess(p => ({ ...p, current: p.current + 1, draft: "" }))}>
                  Next Question →
                </button>
              )}
              {(sess.attempts[sess.current] || []).length > 0 && sess.current === sess.questions.length - 1 && (
                <button style={s.ghostBtn} onClick={() => setSess(p => ({ ...p, step: "results" }))}>
                  See Results →
                </button>
              )}
            </div>
      ...
      {sess.step === "results" && (
        <main style={{ ...s.main, maxWidth: 760 }}>
          <div style={s.resultsHeader}>
            ...
            {avgScore && (
              <div style={s.avgScore}>
                ...
                <span style={s.avgLabel}>avg score</span>
              </div>
            )}
          </div>
          ...
          <button style={{ ...s.primaryBtn, marginTop: 24 }} onClick={() => setSess(FRESH_SESSION)}>
            Start New Session
          </button>
        </main>
      )}
